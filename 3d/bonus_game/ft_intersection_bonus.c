...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/08/21 23:25:06 by bramzil           #+#    #+#             */
/*   Updated: 2024/08/21 23:33:42 by bramzil          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	is_wall(t_data *dt, t_point *p, int x_d, int y_d)
{
	...
	else if (dt->map.arr[j][i] == 'D')
	{
		...
		if (door && (x_d) && \
			(((p->y < (j * dt->grd[1] + door->var)) || \
			(((j * dt->grd[1] + 64) - door->var) < p->y))))
			return (1);
		else if (door && (y_d) && \
			((p->x < (i * dt->grd[0] + door->var)) || \
			(((i * dt->grd[0] + 64) - door->var) < p->x)))
			return (1);
	}
	return (0);
}
